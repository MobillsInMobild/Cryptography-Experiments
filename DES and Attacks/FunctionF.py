...
# IP置换部分，op为0表示使用IP_table正置换，op为1表示使用Inv_IP_table逆置换
def IP(text, op):   
...
def Sbox(text):
    ...
    for i in range(8):
        # 输入的第一和最后一个比特构成一个2位二进制数，用来选择由Si表中的四行所定义的四种替代的一种，中间的4个比特则选出一列
        row=2*int(text[i*6])+int(text[i*6+5])
        column=8*int(text[i*6+1])+4*int(text[i*6+2])+2*int(text[i*6+3])+1*int(text[i*6+4])
        temp=S[i][row*16+column]
        # temp即是s盒转换之后得到的值，然后将转换为4bit数并保存
        for k in range(4):
            result[4*i+k]=HexBinTrans.intTobit(temp)[k]
...
# xor
def xor(bit1,bit2):
    result=[0 for i in range(len(bit1))]
    for i in range(len(bit1)):
        result[i]=str(int(bit1[i])^int(bit2[i]))
    return result
